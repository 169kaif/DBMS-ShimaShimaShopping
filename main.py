...
#OLAP query
def greaterTenMill():
    sql_statement="select customer_name, address, sum(total_cost) AS total_sales from Bill group by customer_name, address with rollup having total_sales>10000000"
    cursor.execute(sql_statement)
    record=cursor.fetchall()
    print(record)
        
#OLAP query
def diffPaymentModes():
    sql_statement="select payment_mode, customer_name, total_cost, sum(total_cost) AS sales_amount from Bill group by payment_mode, customer_name, total_cost with rollup order by sales_amount"
    cursor.execute(sql_statement)
    record=cursor.fetchall()
    print(record) 

#OLAP query
def prodDescInventory():
    sql_statement="select product_description, product_name, sum(cost) from Product group by product_description, product_name with rollup "
    cursor.execute(sql_statement)
    record=cursor.fetchall()
    print(record)

#OLAP query
def sortCustPaymentMode():
    sql_statement="select customer_name, payment_mode, sum(total_cost) from Bill group by customer_name, payment_mode with rollup"
    cursor.execute(sql_statement)
    record=cursor.fetchall()
    print(record)
    ...
    if(x==1):
        ...
        if(u_e,p_e) in record:
            while(True):
                ...
                print("4.High revenue generating customers and the total revenue generated by them(OLAP)")
                print("5.Revenue generated by different payment modes(OLAP)")
                print("6.Information about product description and inventory(OLAP)")
                print("7.Revenue generated by customers with info about their payment modes(OLAP)")
                print("8.Exit")
